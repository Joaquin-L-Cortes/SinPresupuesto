...
function enterEditMode() {
  // Modo edición compacto — mismo tamaño de modal
  const box = document.getElementById('perfil-modal-box');
  // No ampliar el modal, el diseño cabe en el mismo ancho
  tempAvatarId = userProfile.avatarId || 1;
  ...
  // Picker compacto: botones más pequeños para caber en 1 fila de 9
  document.getElementById('edit-avatar-picker').innerHTML = AVATARS.map(a => `
  ...
  <div class="modal-overlay" id="perfil-modal">
    <div class="modal" id="perfil-modal-box" style="max-width:420px;transition:max-width .28s cubic-bezier(.4,0,.2,1);">
      ...
      <div id="perfil-edit" style="display:none">
        ...
        <div class="form-group" style="margin-bottom:.5rem;"><label>Correo</label><input type="email" id="edit-email" inputmode="email"></div>
        <div class="form-group" style="margin-bottom:.5rem;">
          <label>Género</label>
          <select id="edit-genero" style="width:100%;background:var(--bg3);border:1.5px solid var(--border);border-radius:10px;padding:.5rem .9rem;color:var(--text);font-family:'DM Sans',sans-serif;font-size:.88rem;outline:none;">
            <option value="M">Masculino</option><option value="F">Femenino</option>
            <option value="NB">No binario</option><option value="NR">Prefiero no responder</option>
          </select>
        </div>
